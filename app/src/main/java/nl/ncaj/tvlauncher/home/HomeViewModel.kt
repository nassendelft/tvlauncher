  ...
  val categories = appResolver.getLeanbackLaunchApplications()
    ...
    .map { launchApps ->
      listOfNotNull(
        ...
        launchApps.filter { it.isGame }
          .takeIf { it.isNotEmpty() }
          ?.let { LeanbackCategory(label = "Games", apps = it) }
      )
    }
